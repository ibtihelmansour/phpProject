...
/* FIX ECOMBSKA-9487 */
angular.module("itxApp").config(["algoliaSearchConstants", function(constants) {
	constants.keysToCopy.push("removeWordsIfNoResults");
}]);
/* END FIX ECOMBSKA-9487 */

/* FIX ECOMBSKA-9486 */
ItxShopCartPageClass.prototype._setScopeFromShopCart = function() {
	var shopCartService=this.iScope.shopCartService
	
	var promotions = shopCartService.getCurrentPromo();
	if(promotions && promotions.length > 0){
		this.iScope.iPromoCodes = [];
		for (var i = 0; i < promotions.length; i++) {		
			if(promotions[i] && promotions[i].code) {		
				this.iScope.iPromoCodes.push(promotions[i].code);
			}		
		}	
		this.iScope.iHasPromoCode = true;
	} else {		
		this.iScope.iHasPromoCode = false;
	}	

	var shopCart = shopCartService.getShopCart();
	this.iScope.iGiftPacking = shopCart.giftPacking;
	this.iScope.iGiftTicket = shopCart.giftTicket==1;
	this.iScope.iGiftMessage = shopCart.message || "";

	if(shopCartService.getNumberOfItems() == 0){
		// GTM-pageview Cesta vacia
		this.iScope.trackingService.trackPageview({ name:'page-view-shopcart' });
	}

	this.iScope.showShippingPrice = shopCart.shippingPrice !== 0; // CHANGES
};
/* END FIX ECOMBSKA-9486 */

/* FIX ECOMBSKA-9477 */
paymentGiftCardCtrl.prototype.addGiftCard=function() {
	var self=this;
	...
	this.giftCards.add({
		...
		self.cvv2=undefined;
		self.captchaValue=undefined;
		self.captchaId=undefined;
		self.ToS=false;
		self.giftCards.showFields=false;
		// self.giftCard.$setValidity('reset.giftcard', false); // CHANGE
	}, function(error) {
		self.$scope.$broadcast(bershka.events.ReloadCaptcha);
		inditex.xOpenErrorPopUp(278, "auto", inditex.text("modal.errorTitle"), inditex.text("modal.genericError"), null);
		self.giftCards.showFields=true;
	}).finally(function() {
		self.disableButton=false;
	});
};
/* END FIX ECOMBSKA-9477 */
...
/* FIX ECOMBSKA-9312 */
gridProductCtrl.prototype.goToReal = function() {
	var product=this.iProduct;
	
	var selectedColor = this.iSelectedColor
		?this.iSelectedColor
		: product.originalProduct.detail && product.originalProduct.detail.colors 
			? product.originalProduct.detail.colors[0]
			: undefined;
	...
	this.trackingService.trackEvent({
		...
		data:{
			product: product,
			list: 'parrilla',
			position:this.gridProductHelpers.getProductVisibleIndex(product) + 1,
			selectedColor: selectedColor,
			cf:'catalogo',
			eventCategory: 'parrilla',
			action: 'ver_producto',
			imagen: this.iCurrentImage.src
		}
	});
	
	if (this.bIsGiftCard) {
		window.location.href=this.iDetailURL;
	} else {		
		this._updateProductRepository();
		var pos = this.gridProductHelpers.getProductOriginalIndex(product)+1;
		inditex.setSessionStorage("productPosition", pos);
		inditex.setSessionStorage("initialImage", this.iCurrentImage.src);
		var color = selectedColor ? selectedColor : this.shownProduct.colors[0];
		this.navigationFactory.setProduct(this.shownProduct, color, this.categoryFactory.oCategory);
	}	
};
/* END FIX ECOMBSKA-9312 */

/* FIX ECOMBSKA-9305 */
var oldSimulateProgress = ItxAjaxProductPageClass.prototype._bindings.simulateProgress;
ItxAjaxProductPageClass.prototype._bindings.simulateProgress = function(buttonIndex, seconds, callback, event) {
	/* CHANGES */
	var disabled = ['out_of_stock', 'back_soon', 'coming_soon'];
	if (this.iScope.iSize && disabled.indexOf(this.iScope.iSize.stock) != -1) {
		return;
	}
	/* END CHANGES */
	oldSimulateProgress.apply(this, arguments);
};
/* END FIX ECOMBSKA-9305 */

/* FIX ECOMBSKA-9304 */
ItxStockNotificationPageClass.prototype._bindings.sendForm = function() {
	var self = this;
	// Se comprueba si valida el formulario
	if ($('#stockNotificationForm').valid()) {
		var actions = [{
			action: "catalog:stockNotification",
			params: {
				storeId: inditex.iStoreId,
				catalogId: inditex.iCatalogId,
				categoryId: inditex.iCategoryId || 0, // CHANGE
				productId: this.iScope.iProduct.id,
				languageId: inditex.iLangId
			},
			data:{
				sku: this.iScope.iSize.sku,
				quantity: 1,
				name: this.iScope.stockNotName,
				email: this.iScope.stockNotMail,
				pushToken: inditex.iUserJSON.pushToken || undefined
			}
		}];

		if (this.iScope.newsletterStock) {
			actions.push({
		...
		this.executeCache.execute(actions).catch(function(responses) {
			if (self.iScope.newsletterStock && responses[1].state !== self.executeCache.STATE_RESOLVED) {
				self.launchNewsletterEvent('fail', responses[1]);
			}

			if(responses[0].state === self.executeCache.STATE_RESOLVED) {
				return responses;
			}
			return $q.reject(responses);
		}).then(function(responses) {
			self.trackingService.trackEvent({
				name:'coming-soon-sended',
				data:{
					product: self.iScope.iProduct,
					productColor: self.iScope.iColor,
					productSize: self.iScope.iSize,
					type: self.iScope.iSize.stock
				}
			});

			if(self.iScope.newsletterStock && responses[1].state === self.executeCache.STATE_RESOLVED) {
				self.launchNewsletterEvent('success');
			}
			
			// Si la llamada finaliza correctamente, se cierra la modal de notificacion de stock y se muestra la de confirmacion
			var $modalQS = $("#modal-qs");

			if($modalQS.length && $modalQS.html()!= ""){
				$modalQS.dialog('close');
			}else{
				$('#modal').dialog('close');
			}
			...
			self.trackingService.trackEvent({
				name:'error-servidor-stockNotificationForm', 
				data:{
					type: self.iScope.iSize.stock,
					response: e  // CHANGE
				}
		...
		var label = this._getFormErrors();
		this.trackingService.trackEvent({
			name:'error-stockNotificationForm',
			data:{
				label: label,
				type: this.iScope.iSize.stock
			}
		});
	}
};
...
angular.module("itxApp").config(["$provide", function($provide) {
	$provide.decorator("shopCartItemDirective", ["$delegate", "$timeout", function($delegate, $timeout) {
		var directive = $delegate[0];
		var controller = directive.controller.pop();
		directive.controller.push("$timeout", controller);
		
		shopCartItemCtrl.prototype.init = function($attrs, shopCartService, trackingService, imageService, executeCache, navigationFactory, categoryFactory, productNormalizer, imageConstants, configurationFactory, productHelpers, $timeout) {
...
(function() {
	var oldSize,oldColor,oldQuantity;
	shopCartItemCtrl.prototype.$doCheck=function() {
		if(!this.iEditMode) {
			oldSize=oldColor=oldQuantity=undefined;
			return;
		}
		if(this.selectedSize && this.selectedSize!=oldSize) {
			oldSize=this.selectedSize;
			if(this.iEditInProgress == false && (oldSize.stock=="back_soon" || oldSize.stock=="coming_soon")) { // CHANGE
				this.iEditInProgress = true;
				this.openEmailMeModal(oldSize);
			}
		}
		if(this.selectedColor && this.selectedColor!=oldColor) {
			oldColor=this.selectedColor;
			var name=this.selectedSize.name;		
			for(var i=0,len=oldColor.sizes.length;i<len;i++) {
				var size=oldColor.sizes[i];
				if(size.name==name && (size.stock!="back_soon" && size.stock!="coming_soon")) {
					this.selectedSize=size;
					break;
				}
			}
		}
		if(this.selectedQuantity && this.selectedQuantity!=oldQuantity) {
			oldQuantity=this.selectedQuantity;
			if(oldQuantity===0) {
				this.remove();
			} else {
				this._updateRange();
...
shopCartItemCtrl.prototype.openEmailMeModal = function(size) {
	var url = inditex.encodeUrl({
		action : "ItxStockNotificationPage",
		params : {
			storeId : inditex.iStoreId,
	...
	var self=this;
	inditex.popupUrl(url, 648, 'auto', function() {
		inditex.initModalController('itxStockNotificationPage', {
			product: self.iProductDetail,
			size: size,
			imageUrl: self.getProductImage(),
			color: self.selectedColor,
			type: size.stock // CHANGE
		});
			
	}, function(){
		self.$timeout(function () {
			self.setSizeAndColorFromDetail(self.iProductDetail); // CHANGE
			self.iEditInProgress = false; // CHANGE
		}, 0);
		self.trackingService.trackPageview({ name:'page-view-shopcart' });
	}, 'modal');
};

ItxTrackingService.prototype.setComingSoonProductoPageviewData = function() {
	
	var detail = this.product.detail ? this.product.detail : this.product.originalProduct.detail; // CHANGE
	...
	this.dataLayer.customData = {
		'product': {
			'reference': this._getProductReference(detail.reference), // CHANGE
			'productState': detail.productState, // CHANGE
			'claseProducto': this.product.section,
			'stock': this.stock,
			'color': this.productColor.id,
			'size': this.productSize.mastersSizeId,
	...
	this.dataLayer.cart = {
		'cartNumberItems': this.cartNumberItems,
		'cartNumberItemsNoStock': this.cartNumberItemsNoStock,
		'cartProducts': this.getSimpleCartProducts(shopCartService.getShopCartItems())
	};

};

ItxTrackingService.prototype.setComingSoonEventData = function() {

	var detail = this.product.detail ? this.product.detail : this.product.originalProduct.detail; // CHANGES
	this.dataLayer.customData = {
		'product': {
			'color': this.productColor.id,
			'talla': this.productSize.mastersSizeId,
			'partnumber': this.productSize.partnumber,
			'section': undefined, //this.product.section,
			'family': undefined, //this.product.family,
			'subfamily': undefined, //this.product.subFamily,
			'reference': detail.reference, // CHANGES
			'productState': undefined, //this.product.detail.productState,
			'claseProducto': undefined,
			'stock': undefined
		},
		'category': {
			'season': undefined
...
/* END FIX ECOMBSKA-9304 */

/* FIX ECOMBSKA-9313 */
inditex.xGetDeltaPriceHtml = function(oldPrice, newPrice) {
	var deltaPriceHtml = '';

	var deltaPrice = inditex.xGetDeltaPrice(oldPrice, newPrice);
	if(deltaPrice!==undefined && deltaPrice != 0) {
	  deltaPriceHtml= "<span class='delta'><span class='integer'>" + deltaPrice + "</span><span class='delta-symbol'>%</span></span>";
	}
	
	return deltaPriceHtml;
};

inditex.xGetBasicDeltaPriceHtml = function(oldPrice, newPrice) {
	var deltaPriceHtml = '';

	var deltaPrice = inditex.xGetDeltaPrice(oldPrice, newPrice);
	if(deltaPrice!==undefined && deltaPrice != 0) {
	  deltaPriceHtml = "<span class='delta'>" + deltaPrice + "%</span>";
	}
	
	return deltaPriceHtml;
};
/* END FIX ECOMBSKA-9313 */


/* FIX ECOMBSKA-9301 */
bskMapCtrl.prototype.drawMarkers=function() {
	this.closeWindows();
	this.map.clear();

	for (var i = 0, len = this.elements.length; i < len; i++) {
		this.addMarker(this.elements[i]);
	}
};
/* END FIX ECOMBSKA-9301 */
...
angular.module("itxApp").config(["$provide", function($provide) {
	$provide.decorator("bskGridImageState", ["$delegate", function($delegate) {
...
ItxLogonPageClass.prototype._proceed = function() {
	if (this.iScope.iCheckoutButtonAction) {
